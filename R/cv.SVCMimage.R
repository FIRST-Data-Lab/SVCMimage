#' Cross-validation for varying coefficient models with imaging data.
#'
#' This function implements k-fold cross-validation for for varying coefficient models with imaging data, and returns the mean squared prediction error.
#'
...
#' \cr
#' @details This R package is the implementation program for manuscript entitled ``Estimation and Inference for Image-on-Scalar Regression" by Shan Yu, Guannan Wang, Li Wang and Lijian Yang.
#'
#' @examples
...
#' dat=data.SVCMimage(n,Z,ind.inside,sigma,rho,2019,lambda1,lambda2)
#' Y=dat$Y; X=dat$X; Z=dat$Z;
#' Y=Y[,ind.inside] # remove the points which are outside the boundary;
#' lambda=10^(seq(-6,6,by=1))
#' cv.SVCMimage(Y,X,Z,V,Tr,d,r,lambda,nfold,iter)
#'
#' @export
#'
cv.SVCMimage <- function(Y,X,Z,V,Tr,d=5,r=1,lambda=10^seq(-6,6,by=0.5),nfold=10,iter=2019){
  if(nfold<3){
  ...
  for(ii in 1:nfold){
    ...
    if(d>1){
      mfit.ii <- SVCMimage.est.ho(B,Q2,K,X.train,Y.train,lambda)
      nl <- length(lambda)
    ...
    if(d== -1){
      mfit.ii <- SVCMimage.est.pc(B,X.train,Y.train)
