...
#' \cr
#' @return An object with S3 class "SVCMimage", including
...
#'
#' @details This R package is the implementation program for manuscript entitled ``Multivariate Spline Estimation and Inference for Varying Coefficient Models with Imaging Data" by Shan Yu, Guannan Wang, Li Wang and Lijian Yang.
#'
#' @examples
...
#' cv=cv.SVCMimage(Y,X,Z,V,Tr,d,r,lambda,nfold)
#' lamc=cv$lamc
#' mfit0=fit.SVCMimage(Y,X,Z,V,Tr,d,r,lamc)
#' @export
#'
fit.SVCMimage <- function(Y,X,Z,V,Tr,d=5,r=1,lambda=0.1){
  if(!is.matrix(Y)){
  ...
  if(d>1){
    mfit <- SVCMimage.est.ho(B,Q2,K,X,Y,lambda)
  }
  if(d== -1){
    mfit <- SVCMimage.est.pc(B,X,Y)
  }
