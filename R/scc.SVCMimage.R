#' Simultaneous confidence corridors for multivariate varying coefficient models
#'
...
#' @export
#'
scc.SVCMimage <- function(Y,X,Z,V,Tr,d=5,r=1,lambda=10^seq(-6,6,by=0.5),
  ...
  if(d>1){
    CC <- SVCMimage.CC.ho(B,Q2,K,X,Y,lambda,alpha0,nboot,ksi)
  }
  if(d== -1){
    CC <- SVCMimage.CC.pc(B,X,Y,tria.all,alpha0,nboot,ksi)
  }
  return(CC)
}
